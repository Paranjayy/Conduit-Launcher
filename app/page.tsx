...
export default function Home() {
  ...
  useHotkeys("meta+shift+f", () => {
    setActiveView("fileSearch");
    setIsOpen(true);
  });

  // Toggle focus session
  useHotkeys("meta+shift+t", () => {
    setActiveView("focusSession");
    setIsOpen(true);
  });

  // Toggle window manager
  useHotkeys("meta+shift+w", () => {
    setActiveView("windowManager");
    setIsOpen(true);
  });
  ...
  useEffect(() => {
    ...
    if (
      isElectron &&
      window.electron &&
      window.electron.shortcuts?.onGlobalShortcut
    ) {
      // @ts-ignore
      const cleanup = window.electron.shortcuts.onGlobalShortcut(
        (id: string) => {
          switch (id) {
            case "notes":
              setActiveView("notes");
    ...
    return (
      <main className="flex flex-col h-screen bg-background">
        ...
        <div className="flex-1 overflow-hidden">
          {activeView === "command" && (
            <CommandBar onViewChange={handleViewChange} />
          )}
          {activeView === "notes" && (
            <NotesManager onViewChange={handleViewChange} />
          )}
          {activeView === "multiClipboard" && (
            <MultiClipboard onViewChange={handleViewChange} />
          )}
          {activeView === "aiChat" && (
            <AIChat onViewChange={handleViewChange} />
          )}
          {activeView === "emojiSearch" && (
            <EmojiSearch onViewChange={handleViewChange} />
          )}
          {activeView === "clipboard" && (
            <ClipboardManager onViewChange={handleViewChange} />
          )}
          {activeView === "pasteStack" && (
            <PasteStack onViewChange={handleViewChange} />
          )}
          {activeView === "snippets" && (
            <SnippetsManager onViewChange={handleViewChange} />
          )}
          {activeView === "appSearch" && (
            <AppSearch onViewChange={handleViewChange} />
          )}
          {activeView === "calculator" && (
            <Calculator onViewChange={handleViewChange} />
          )}
          {activeView === "preferences" && (
            <Preferences onViewChange={handleViewChange} />
          )}
          {activeView === "contextualShortcuts" && (
            <ContextualShortcuts onClose={() => setActiveView("command")} />
          )}
          {activeView === "menuSearch" && (
            <MenuSearch onViewChange={handleViewChange} />
          )}
          {activeView === "fileSearch" && (
            <FileSearch onViewChange={handleViewChange} />
          )}
          {activeView === "focusSession" && (
            <FocusSession onViewChange={handleViewChange} />
          )}
          {activeView === "windowManager" && (
            <WindowManager onViewChange={handleViewChange} />
          )}
        </div>
      </main>
    );
  ...
  return (
    <main className="fixed inset-0 flex items-center justify-center bg-black/50 backdrop-blur-sm z-50">
      <div className="bg-background rounded-lg shadow-lg overflow-hidden w-[1000px] max-w-[95vw] h-[700px] max-h-[90vh] border border-border/50 glass-panel">
        {activeView === "command" && (
          <CommandBar onViewChange={handleViewChange} />
        )}
        {activeView === "notes" && (
